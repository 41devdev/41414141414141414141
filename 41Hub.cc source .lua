...
if script_key then
    writefile("41Hubcc_Key.txt", script_key)
end
...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("41hub.cc | Security : You are running another script, please disable it and execute again")
    end
end
    ...
    LPH_JIT_MAX(function()
        ...
        if Game_Name == "South Bronx" then
            ...
            HideUI = LPH_NO_VIRTUALIZE(function(Title, Timing)
                ...
                textLabel.Text = '<font color="rgb(0,163,224)">41Hub</font>cc\n' .. Title
...
local window = library:window({name = "41Hub.", suffix = "cc", gameInfo = string.format("41Hub.cc : %s", Game_Name:lower())})
